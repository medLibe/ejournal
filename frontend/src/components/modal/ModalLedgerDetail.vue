...
export default {
    ...
    emits: ['fetchLedgerDetails', 'update:isVisible'],
    data() {
        return {
    ...
    methods: {
        ...
        async onSubmitFilter() {
            ...
            this.$api.get(`${import.meta.env.VITE_API_URL}/report/ledger-detail`, {
                    params,
                    headers: {
                ...
                .then(response => {
                    if (response.data.status) {
                        ...
                        // emit data
                        this.$emit('fetchLedgerDetails', response.data.data.ledgers, response.data.data.totals)
