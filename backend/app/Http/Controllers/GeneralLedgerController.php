...
class GeneralLedgerController extends Controller
{
    ...
    public function getGeneralLedgers(Request $request, $importNo)
    {
        try {
            $import = $this->generalLedgerImport->where('import_no', $importNo)->first();
            ...
            $getGeneralLedgers = $this->generalLedger->getGeneralLedgers($import->id);

            return response()->json([
                'status'         => true,
                'generalLedgers' => $getGeneralLedgers,
                'total'          => count($getGeneralLedgers)
            ]);
    ...
    // to validate account and prepare data for general ledger
    ...
    private function updateAccountBalance($row, $openingBalance)
    {
        if ($row['normal_balance'] == 1) { // Normal Balance Debit
            if ($row['tipe_transaksi'] === 1) { // Debit
                return $openingBalance + $row['amount'];
            } else { // Kredit
                return $openingBalance - $row['amount'];
            }
        } elseif ($row['normal_balance'] == 2) { // Normal Balance Kredit
            if ($row['tipe_transaksi'] === 2) { // Kredit
                return $openingBalance + $row['amount'];
            } else { // Debit
                return $openingBalance - $row['amount'];
            }
