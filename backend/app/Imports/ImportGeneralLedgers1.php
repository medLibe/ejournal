...
class ImportGeneralLedgers implements ToCollection, WithHeadingRow, WithChunkReading
{
    protected $allProcessed = [];
    protected $isFirstChunk = true;
    
    public function collection(Collection $collection)
    {
        // if check header once
        if ($this->isFirstChunk) {
            $this->validateHeaders($collection->first());
        ...
        foreach ($collection as $row) {
            ...
            try {
                $this->allProcessed[] = $this->processRow($row);
            } catch (Exception $e) {
                throw $e;
            }
        }
    ...
    {
        $required = [
            'tanggal', 'nomor_akun', 'department', 'nomor_sumber',
            'tipe_sumber', 'debit', 'kredit', 'keterangan'
        ];

        foreach ($required as $column) {
            if (!array_key_exists($column, $headers)) {
                throw new Exception("Kolom '$column' tidak ditemukan di dalam file.", 422);
            }
        }
    }
    ...
    private function processRow($row)
    {
        ...
        $account = Account::where('account_code', $row['nomor_akun'])->first();

        if (!$account) {
            throw new \Exception("Nomor akun '{$row['nomor_akun']}' tidak ditemukan.", 409);
        }

        if ($account->is_parent) {
            throw new \Exception("Akun '{$row['nomor_akun']}' adalah akun parent dan tidak dapat digunakan.");
        }
        ...
        if (!empty($row['debit']) && $row['debit'] != 0) {
            ...
            throw new \Exception("Nomor akun '{$row['nomor_akun']}' tidak memiliki debit/kredit.");
        }
